...
compile:
	...
	@if [ "$(MODE)" = "pthreads" ]; then \
		$(CC) $(CFLAGS) $(CFLAGS_POLYBENCH) utilities/polybench.c stencils/seidel-2d_pthreads.c -o $(EXECUTABLES_DIR)/seidel-2d_pthreads -D$(SIZE)_DATASET; \
		...
		$(MPICC) $(CFLAGS) $(CFLAGS_POLYBENCH) utilities/polybench.c stencils/seidel-2d_mpi.c -o $(EXECUTABLES_DIR)/seidel-2d_mpi -D$(SIZE)_DATASET; \
	else \
